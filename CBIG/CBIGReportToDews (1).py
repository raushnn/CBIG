...
class AutoReportToDews:
        ...
        try:
            ...
            if (action == "Upload File"):
                    ...
                    for file in os.listdir(OutPutFilesReportToDews["OutPutFilesReportToDews"]):
                        try:
                            ...
                            target_folder = ctx.web.get_folder_by_server_relative_url(
                                folder_url)
                            path = os.path.join(
                                OutPutFilesReportToDews["OutPutFilesReportToDews"],  file)
                ...
                try:
                    ...
                    folders = ctx.web.get_folder_by_server_relative_url(
                        folder_url).folders
                    ctx.load(folders)
                    ctx.execute_query()
                    FileListCount = []
                    ...
                    try:
                        ...
                        for folder,  i in zip(folders,  range(0,  len(folders))):
                            ...
                            try:
                                ...
                                ctx.execute_query()
                                if (len(files) > 0):
                                    List = [FolderName,  len(files),  "File Available",  str(
                                        today) + "_" + str(logtime)]
                                else:
                                    List = [FolderName,  len(files),  "Folder Empty",  str(
                                        today) + "_" + str(logtime)]
                    ...
                    if folders:
                        ...
                        elapsed = time.strftime(
                            "%H:%M:%S %Z",  time.gmtime(t1 - t0))
                        logger.info("Files has been downloaded")
                        logger.info(
                            "Total time to download the files is " + str(elapsed))
                ...
                try:
                    ...
                    for file,  i in zip(files,  range(0,  len(files))):
                        try:
                            TimeCreated = (
                                str(file.properties['TimeCreated']).split('T')[0])
                            TimeCreated = datetime.strptime(
                                TimeCreated,  '%Y-%m-%d')
                            YesterdayDate = datetime.strptime(
                                Yesterday["Yesterday"],  '%Y-%m-%d')
                            todaydate = datetime.strptime(
                                Today["Today"],  '%Y-%m-%d')
                            ...
                            if duration > 7:
                                ...
                                ctx.execute_query()
                                logger.info(
                                    "CSV File was deleted since more duration is greater than 7 days")
                            # else:
                ...
                try:
                    ...
                    for file,  i in zip(files,  range(0,  len(files))):
                        try:
                            ...
                            timecreated = datetime.strptime(
                                timecreated,  '%Y-%m-%d')
                            sevendaysold = datetime.strptime(
                                SevenDaysOldFile["SevenDaysOldFile"],  '%Y-%m-%d')
                            todaydate = datetime.strptime(
                                Today["Today"],  '%Y-%m-%d')
    ...
    def UpdateFieldIDDataframes(self,  df,  logger):
        try:
            ...
            if "Mgmt" in sheetname:
                ...
                # FIELDS Loops
                df['Fieldid'] = df.groupby(
                    ['duns_no', 'Indian Bank Name_Bank:bb']).cumcount() + 1
                ...
                if 'SIC' in sheetname:
                    df['SIC extension_General:ce'] = df['SIC extension_General:ce'].astype(
    ...
    def validate_data(self, df, dffielddict, logger):
        try:
            ...
            if "Mgmt" in sheetname:
                ...
                df['Identity Number_Mgmtbac:EL'] = df['Identity Number_Mgmtbac:EL'].apply(
    ...
    def autofinnonfin(self,  logger):
        ...
        dataAll = [
            ['General non loop',  'General non loop',  'NL', 'N', 'A:Z'],
            ['Gen_Insurance',  'Gen_Insurance',  'NL', 'Y', 'A:C'],
            ['Gen_Award',  'Gen_Award', 'L', 'Y', 'A:G'], 
            ['Gen_Membership',  'Gen_Membership', 'L', 'Y', 'A:F'], 
            ['Gen_Registration',  'Gen_Registration', 'L', 'Y', 'A:D'], 
            ['Gen_ISOCert', 'Gen_ISOCert', 'L', 'Y', 'A:G'], 
            ['Gen_Product', 'Gen_Product', 'L', 'Y', 'A:C'],
            ['Gen_CustomerTable', 'Gen_CustomerTable', 'L', 'Y', 'A:F'], 
            ['Gen_ExportTerms', 'Gen_ExportTerms', 'L', 'Y', 'A:E'], 
            ['Gen_LocalSalesTerms', 'Gen_LocalSalesTerms', 'L', 'Y', 'A:E'], 
            ['Gen_CountrywiseExport', 'Gen_CountrywiseExport', 'L', 'Y', 'A:E'], 
            ['Gen_localPurchTerms', 'Gen_localPurchTerms', 'L', 'Y', 'A:E'], 
            ['gen_ImportTerms', 'gen_ImportTerms', 'L', 'Y', 'A:E'], 
            ['Gen_Countrywise Purchase', 'Gen_Countrywise Purchase', 'L', 'Y', 'A:E'], 
            ['General_Source', 'General_Source', 'L', 'N', 'A:D'], 
            ['General_Telephone', 'General_Telephone', 'L', 'Y', 'A:F'], 
            ['Gen_SIC', 'Gen_SIC', 'L', 'Y', 'A:D'], 
            ['History Non loop', 'History Non loop','NL', 'N', 'A:AA'], 
            ['History Address', 'History Address', 'L', 'Y', 'A:H'], 
            ['History_Background', 'History_Background', 'L', 'Y', 'A:G'], 
            ['History_auditor', 'History_auditor', 'L', 'Y', 'A:C'], 
            ['History_StockExchange', 'History_StockExchange', 'L', 'Y', 'A:C'], 
            ['History_Authorize', 'History_Authorize', 'L', 'Y', 'A:D'], 
            ['History_Issued', 'History_Issued', 'L', 'Y', 'A:D'], 
            ['History_Paidup', 'History_Paidup', 'L', 'Y', 'A:F'], 
            ['History_Sharehold', 'History_Sharehold', 'L', 'Y', 'A:G'], 
            ['Mgmt Loop_Non Loop', 'Mgmt Loop_Non Loop', 'NL', 'Y', 'A:K'], 
            ['Relcn Non loop', 'Relcn Non loop', 'NL', 'N', 'A:U'], 
            ['Rel_Subsidiary', 'Rel_Subsidiary', 'L', 'Y', 'A:K'], 
            ['Rel_Affiliate', 'Rel_Affiliate', 'L', 'Y', 'A:M'], 
            ['Rel_Branch', 'Rel_Branch', 'L', 'Y', 'A:K'], 
            ['Cinvest Loop', 'Cinvest Loop', 'NL', 'Y', 'A:C'], 
            ['Bank Loop_Non Loop', 'Bank Loop_Non Loop', 'L', 'Y', 'A:J'], 
            ['Supplier looping', 'Supplier looping', 'L', 'Y', 'A:G']
        ]

        #making dataAll list dynamic
        #taking a samplefile and putting data like there
        filepath= "C:\\Users\\khannapo\\OneDrive - Dun and Bradstreet\\Documents\\CBIG\\InputFiles\\ReportToDews\\Sample India Limited_ReportToDews_Template.xlsx"
        ...
        for sheet in f.sheet_names:
            ...
            for j in range(len(dataAll)):
                if str(sheet) in dataAll[j]:
                    index=j
                    break
            if index!= None:
                dataAll[index].append(column_name)
                dataAll[index].append({column_name[0]:np.int64})

        

        try:
            df_FinNonFin = pd.DataFrame(dataAll,  columns=[
                                        'Sheets', 'SheetName',  'Looping/NonLooping',  'Deletion',  'CellRange', 'ColumnList', 'dtype'])
        ...
        try:
            logger.info("Process Started " +
                        str(datetime.now().strftime('%d_%m_%Y %H_%M_%S')))

            Message,  AutoFinancialObj = self.sharepointoperation(
                "Delete File",  "",  logger,  "")
            if ('CSV Files have been deleted' in Message):
                logger.info(
                    "Step 1 : CSV Files older than 7 days have been deleted")
            Message,  AutoFinancialObj = self.sharepointoperation(
                "Download File",  "",  logger,  "")
            if os.listdir(Inputdirectory) != []:
                logger.info(
                    "Step 1 : Folder has been downloaded from sharepoint")
            else:
                ...
                try:
                    ...
                    for (index,  colname) in df_FinNonFin.iterrows():
                        try:
                            ...
                            if key == "General non loop":
                                ...
                                if DataMatchesWitUnity == 0:
                                    ErrorMessage = ErrorMessage + \
                                        ",  Duns Number and Legal Name did not match the Unity Extract"
                            ...
                            os.remove(
                                InPutFilesFinancial["InPutFilesFinancial"] + filename)
                            logger.error(str(ex))
                            continue
            ...
            for file in os.listdir(Inputdirectory):
                ...
                try:
                    ...
                    sheetname_dict={'Gen_Insurance':Gen_Insurance_All,'Gen_Award': Gen_Award_All,'Gen_Membership':Gen_Membership_All,'Gen_Registration':Gen_Registration_All,
                                    'Gen_TradeStyle':Gen_TradeStyle_All,'Gen_ProvFins':Gen_ProvFins_All,'Gen_ISOCert':Gen_ISOCert_All,'Gen_Product':Gen_Product_All,'Gen_Insurance':Gen_Insurance_All,
                                    'Gen_CustomerTable':Gen_CustomerTable_All,'Gen_ExportTerms':Gen_ExportTerms_All, 'Gen_LocalSalesTerms':Gen_LocalSalesTerms_All,
                                    'Gen_CountrywiseExport':Gen_CountrywiseExport_All, 'Gen_localPurchTerms':Gen_localPurchTerms_All,'gen_ImportTerms':gen_ImportTerms_All,
                                    'Gen_Countrywise Purchase':Gen_CountrywisePurchase_All,'General_Source': General_Source_All, 'General_Telephone':General_Telephone_All,
                                    'Gen_SIC':Gen_SIC_All,'History Non loop':History_Non_loop_All,'History Address':History_Address_All,'History_Background': History_Background_All,
                                    'History_auditor':History_auditor_All,'History_StockExchange':History_StockExchange_All,'History_Authorize':History_Authorize_All,'History_Issued':History_Issued_All,
                                    'History_Paidup':History_Paidup_All, 'History_Sharehold':History_Sharehold_All,'Mgmt Loop_Non Loop':Mgmt_Loop_Non_Loop_All,'Relcn Non loop':Relcn_Non_loop_All,
                                    'Rel_Subsidiary':Rel_Subsidiary_All, 'Rel_Affiliate':Rel_Affiliate_All,'Rel_Branch':Rel_Branch_All, 'Cinvest Loop':Cinvest_Loop_All,
                                    'Bank Loop_Non Loop':Bank_Loop_NonLoop_All,'Supplier looping':Supplier_looping_All}

                    sheetname_dict_del={'Gen_Insurance':Gen_Insurance_All_Delete,'Gen_Award': Gen_Award_All_Delete,'Gen_Membership':Gen_Membership_All_Delete,'Gen_Registration':Gen_Registration_All_Delete,
                                    'Gen_TradeStyle':Gen_TradeStyle_All_Delete,'Gen_ProvFins':Gen_ProvFins_All_Delete,'Gen_ISOCert':Gen_ISOCert_All_Delete,'Gen_Product':Gen_Product_All_Delete,'Gen_Insurance':Gen_Insurance_All_Delete,
                                    'Gen_CustomerTable':Gen_CustomerTable_All_Delete,'Gen_ExportTerms':Gen_ExportTerms_All_Delete, 'Gen_LocalSalesTerms':Gen_LocalSalesTerms_All_Delete,
                                    'Gen_CountrywiseExport':Gen_CountrywiseExport_All_Delete, 'Gen_localPurchTerms':Gen_localPurchTerms_All_Delete,'gen_ImportTerms':gen_ImportTerms_All_Delete,
                                    'Gen_Countrywise Purchase':Gen_Countrywise_Purchase_All_Delete, 'General_Telephone':General_Telephone_All_Delete,'Gen_SIC':Gen_SIC_All_Delete,'History Address':History_Address_All_Delete,'History_Background': History_Background_All_Delete,
                                    'History_auditor':History_auditor_All_Delete,'History_StockExchange':History_StockExchange_All_Delete,'History_Authorize':History_Authorize_All_Delete,'History_Issued':History_Issued_All_Delete,
                                    'History_Paidup':History_Paidup_All_Delete, 'History_Sharehold':History_Sharehold_All_Delete,'Mgmt Loop_Non Loop':Mgmt_Loop_Non_Loop_All_Delete,
                                    'Rel_Subsidiary':Rel_Subsidiary_All_Delete, 'Rel_Affiliate':Rel_Affiliate_All_Delete,'Rel_Branch':Rel_Branch_All_Delete, 'Cinvest Loop':Cinvest_Loop_All_Delete,
                                    'Bank Loop_Non Loop':Bank_Loop_Non_Loop_All_Delete,'Supplier looping':Supplier_looping_All_Delete}
                    for key in file_dict:
                        try:
                            ...
                            if not file_dict[key].empty:
                                ...
                                name = SheetDataFrame
                                SheetDataFrame.rename(
                                    columns={'DUNS_General:aa': 'duns_no'}, inplace=True)

                                if not name.empty:

                                    if (name.at[0,  "SheetName"] == 'General non loop'):
                                        Generalnonloop_All = pd.concat(
                                            [Generalnonloop_All, name], axis=0)
                                    else:
                                        sheetname_dict[name.at[0,  "SheetName"]] = sheetname_dict[name.at[0,  "SheetName"]].append(name)
                                    Status = "File Processed"
                                    ...
                                    if (df_FinNonFin.at[name.at[0,  "SheetName"],  "Deletion"] == "Y"):
                                        ...
                                        s = pd.DataFrame(
                                            name.duns_no.unique(), columns=['duns_no'])
                                        if name.at[0,  "SheetName"] in sheetname_dict_del:
                                            sheetname_dict_del[name.at[0,  "SheetName"]].append(s)
                                ...
                                logger.info(
                                    "{} worksheet is empty".format(str(key)))
                                Status = "File Processed"
                                StatsList.append(
                                    ["",  FileWithoutExtn,  str(key),  Status,
                                     str(today) + "_" + str(logtime),
            ...
            if not Generalnonloop_All.empty:
                Generalnonloop_All = self.validate_data(Generalnonloop_All, {'International_General:ev': 'Int64',
            ...
            if not History_Non_loop_All.empty:
                ...
                History_Non_loop_All['Shareholding as at_History:pc'] = pd.to_datetime(
                    History_Non_loop_All['Shareholding as at_History:pc'], errors='coerce')
                History_Non_loop_All['Shareholding as at_History:pc'] = History_Non_loop_All['Shareholding as at_History:pc'].dt.strftime(
                    '%Y-%m-%d')
                ...
                History_Non_loop_All = History_Non_loop_All.astype({'Ordinary Shares (number)_History:gw': 'Int64',
            ...
            if not Relcn_Non_loop_All.empty:
                Relcn_Non_loop_All = Relcn_Non_loop_All.astype({'DUNS_Relcon:DE': 'Int64',
            ...
            if not Gen_Membership_All.empty:
                ...
                Gen_Membership_All.to_csv(
                    Outputdirectory + "Gen_Membership_All_"+todaysDate +
                    "_"+TokenDict["Gen_Membership_Insert"]+".csv",
                    index=False)
            if not Gen_Registration_All.empty:
                self.UpdateFieldIDDataframes(Gen_Registration_All,  logger)
                Gen_Registration_All.to_csv(
                    Outputdirectory + "Gen_Registration_All_"+todaysDate +
                    "_"+TokenDict["Gen_Registration_Insert"]+".csv",
                    index=False)
            ...
            if not Gen_ISOCert_All.empty:
                ...
                Gen_ISOCert_All.to_csv(
                    Outputdirectory + "Gen_ISOCert_All_"+todaysDate +
                    "_"+TokenDict["Gen_ISOCert_Insert"]+".csv",
                    index=False)
            if not Gen_Product_All.empty:
                self.UpdateFieldIDDataframes(Gen_Product_All,  logger)
                Gen_Product_All.to_csv(
                    Outputdirectory + "Gen_Product_All_"+todaysDate +
                    "_"+TokenDict["Gen_Product_Insert"]+".csv",
                    index=False)
            if not Gen_CustomerTable_All.empty:
                self.UpdateFieldIDDataframes(Gen_CustomerTable_All,  logger)
                Gen_CustomerTable_All = self.validate_data(
                    Gen_CustomerTable_All, {'Length of relationship_General:PR': 'Int64'}, logger)
            ...
            if not Gen_LocalSalesTerms_All.empty:
                ...
                Gen_LocalSalesTerms_All = Gen_LocalSalesTerms_All.astype({'Net days_General:eh': 'Int64',
                                                                          'To net days_General:ei': 'Int64'},
                                                                         errors='ignore')
                Gen_LocalSalesTerms_All.to_csv(
                    Outputdirectory + "Gen_LocalSalesTerms_All_"+todaysDate +
                    "_"+TokenDict["Gen_LocalSalesTerms_Insert"]+".csv",
                    index=False)
            if not Gen_CountrywiseExport_All.empty:
                self.UpdateFieldIDDataframes(
                    Gen_CountrywiseExport_All,  logger)
                Gen_CountrywiseExport_All.to_csv(
                    Outputdirectory + "Gen_CountrywiseExport_All_"+todaysDate +
                    "_"+TokenDict["Gen_CountrywiseExport_Insert"]+".csv",
                    index=False)
            if not Gen_localPurchTerms_All.empty:
                ...
                Gen_localPurchTerms_All = Gen_localPurchTerms_All.astype({'Net days_General:fh': 'Int64',
            ...
            if not Gen_CountrywisePurchase_All.empty:
                self.UpdateFieldIDDataframes(
                    Gen_CountrywisePurchase_All,  logger)
                Gen_CountrywisePurchase_All.to_csv(
                    Outputdirectory + "Gen_CountrywisePurchase_All_"+todaysDate +
                    "_"+TokenDict["Gen_Countrywise_Purchase_Insert"]+".csv",
                    index=False)
            if not General_Telephone_All.empty:
                ...
                General_Telephone_All.to_csv(
                    Outputdirectory + "General_Telephone_All_"+todaysDate +
                    "_"+TokenDict["General_Telephone_Insert"]+".csv",
                    index=False)
            if not Gen_SIC_All.empty:
                self.UpdateFieldIDDataframes(Gen_SIC_All,  logger)
                Gen_SIC_All.to_csv(
                    Outputdirectory + "Gen_SIC_All_"+todaysDate +
                    "_"+TokenDict["Gen_SIC_Insert"]+".csv",
                    index=False)
            if not History_Address_All.empty:
                self.UpdateFieldIDDataframes(History_Address_All,  logger)
                History_Address_All.to_csv(
                    Outputdirectory + "History_Address_All_"+todaysDate +
                    "_"+TokenDict["History_Address_Insert"]+".csv",
                    index=False)
            if not History_Background_All.empty:
                ...
                History_Background_All.to_csv(
                    Outputdirectory + "History_Background_All_"+todaysDate +
                    "_"+TokenDict["History_Background_Insert"]+".csv",
                    index=False)
            if not History_auditor_All.empty:
                self.UpdateFieldIDDataframes(History_auditor_All,  logger)
                History_auditor_All.to_csv(
                    Outputdirectory + "History_auditor_All_"+todaysDate +
                    "_"+TokenDict["History_auditor_Insert"]+".csv",
                    index=False)
            if not History_StockExchange_All.empty:
                self.UpdateFieldIDDataframes(
                    History_StockExchange_All,  logger)
                History_StockExchange_All.to_csv(
                    Outputdirectory + "History_StockExchange_All_"+todaysDate +
                    "_"+TokenDict["History_StockExchange_Insert"]+".csv",
                    index=False)
            if not History_Authorize_All.empty:
                self.UpdateFieldIDDataframes(History_Authorize_All,  logger)
                History_Authorize_All.to_csv(
                    Outputdirectory + "History_Authorize_All_"+todaysDate +
                    "_"+TokenDict["History_Authorize_Insert"]+".csv",
                    index=False)
            if not History_Issued_All.empty:
                self.UpdateFieldIDDataframes(History_Issued_All,  logger)
                History_Issued_All.to_csv(
                    Outputdirectory + "History_Issued_All_"+todaysDate +
                    "_"+TokenDict["History_Issued_Insert"]+".csv",
                    index=False)
            if not History_Paidup_All.empty:
                self.UpdateFieldIDDataframes(History_Paidup_All,  logger)
                History_Paidup_All = self.validate_data(
                    History_Paidup_All, {'Amount_History:hj': 'Int64'}, logger)
                History_Paidup_All.to_csv(
                    Outputdirectory + "History_Paidup_All_"+todaysDate +
                    "_"+TokenDict["History_Paidup_Insert"]+".csv",
                    index=False)
            if not History_Sharehold_All.empty:
                self.UpdateFieldIDDataframes(History_Sharehold_All,  logger)
                History_Sharehold_All = History_Sharehold_All.astype({'Ord shares held(number)_History:ik': 'Int64',
                ...
                History_Sharehold_All.to_csv(
                    Outputdirectory + "History_Sharehold_All_"+todaysDate +
                    "_"+TokenDict["History_Sharehold_Insert"]+".csv",
            ...
            if not Mgmt_Loop_Non_Loop_All.empty:
                ...
                Mgmt_Loop_Non_Loop_All = self.validate_data(Mgmt_Loop_Non_Loop_All, {
                                                            'CEO_Mgmtbac:EC': 'Int64', 'Identity Number_Mgmtbac:EL': 'Int64'}, logger)
                Mgmt_Loop_Non_Loop_All['Identity Number_Mgmtbac:EL'].replace(
                    '0000<NA>', '', inplace=True)
                Mgmt_Loop_Non_Loop_All.to_csv(
                    Outputdirectory + "Mgmt_Loop_Non_Loop_All_"+todaysDate +
                    "_"+TokenDict["Mgmt_Loop_Non_Loop_Insert"]+".csv",
                    index=False)
            if not Rel_Subsidiary_All.empty:
                self.UpdateFieldIDDataframes(Rel_Subsidiary_All,  logger)
                Rel_Subsidiary_All.to_csv(
                    Outputdirectory + "Rel_Subsidiary_All_"+todaysDate +
                    "_"+TokenDict["Rel_Subsidiary_Insert"]+".csv",
                    index=False)
            if not Rel_Affiliate_All.empty:
                ...
                Rel_Affiliate_All.to_csv(
                    Outputdirectory + "Rel_Affiliate_All_"+todaysDate +
                    "_"+TokenDict["Rel_Affiliate_Insert"]+".csv",
                    index=False)
            if not Rel_Branch_All.empty:
                self.UpdateFieldIDDataframes(Rel_Branch_All,  logger)
                Rel_Branch_All = self.validate_data(
                    Rel_Branch_All, {'Post code_Relcon:yf': 'Int64'}, logger)
                Rel_Branch_All.to_csv(
                    Outputdirectory + "Rel_Branch_All_"+todaysDate +
                    "_"+TokenDict["Rel_Branch_Insert"]+".csv",
                    index=False)
            if not Bank_Loop_NonLoop_All.empty:
                self.UpdateFieldIDDataframes(Bank_Loop_NonLoop_All,  logger)
                # Post code_Bank: bh
                Bank_Loop_NonLoop_All = self.validate_data(
                    Bank_Loop_NonLoop_All, {'Post code_Bank:bh': 'Int64'}, logger)
                Bank_Loop_NonLoop_All.to_csv(
                    Outputdirectory + "Bank_Loop_NonLoop_All_"+todaysDate +
                    "_"+TokenDict["Bank_Loop_Non_Loop_Insert"]+".csv",
                    index=False)
            if not Supplier_looping_All.empty:
                self.UpdateFieldIDDataframes(Supplier_looping_All,  logger)
                Supplier_looping_All['Report date_Supplier:la'] = pd.to_datetime(
                    Supplier_looping_All['Report date_Supplier:la'], errors='coerce')
                Supplier_looping_All['Report date_Supplier:la'] = Supplier_looping_All['Report date_Supplier:la'].dt.strftime(
                    '%Y-%m-%d')
                Supplier_looping_All.to_csv(
                    Outputdirectory + "Supplier_looping_All_"+todaysDate +
                    "_"+TokenDict["Supplier_looping_Insert"]+".csv",
                    index=False)
            ...
            if not Gen_Insurance_All_Delete.empty:
                ...
                Gen_Insurance_All_Delete.to_csv(
                    Outputdirectory+"Gen_Insurance_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Insurance_Delete"]+".csv")
            if not Gen_Award_All_Delete.empty:
                Gen_Award_All_Delete.dropna(how='all', inplace=True)
                Gen_Award_All_Delete.to_csv(
                    Outputdirectory+"Gen_Award_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Award_Delete"]+".csv")
            if not Gen_Membership_All_Delete.empty:
                Gen_Membership_All_Delete.dropna(how='all', inplace=True)
                Gen_Membership_All_Delete.to_csv(
                    Outputdirectory + "Gen_Membership_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Membership_Delete"]+".csv")
            if not Gen_Registration_All_Delete.empty:
                Gen_Registration_All_Delete.dropna(how='all', inplace=True)
                Gen_Registration_All_Delete.to_csv(
                    Outputdirectory + "Gen_Registration_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Registration_Delete"]+".csv")
            if not Gen_TradeStyle_All_Delete.empty:
                Gen_TradeStyle_All_Delete.dropna(how='all', inplace=True)
                Gen_TradeStyle_All_Delete.to_csv(
                    Outputdirectory + "Gen_TradeStyle_All_Delete_"+todaysDate+"_"+TokenDict["Gen_TradeStyle_Delete"]+".csv")
            if not Gen_ProvFins_All_Delete.empty:
                Gen_ProvFins_All_Delete.dropna(how='all', inplace=True)
                Gen_ProvFins_All_Delete.to_csv(
                    Outputdirectory + "Gen_ProvFins_All_Delete_"+todaysDate+"_"+TokenDict["Gen_ProvFins_Delete"]+".csv")
            if not Gen_ISOCert_All_Delete.empty:
                Gen_ISOCert_All_Delete.dropna(how='all', inplace=True)
                Gen_ISOCert_All_Delete.to_csv(
                    Outputdirectory + "Gen_ISOCert_All_Delete_"+todaysDate+"_"+TokenDict["Gen_ISOCert_Delete"]+".csv")
            if not Gen_Product_All_Delete.empty:
                Gen_Product_All_Delete.dropna(how='all', inplace=True)
                Gen_Product_All_Delete.to_csv(
                    Outputdirectory + "Gen_Product_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Product_Delete"]+".csv")
            if not Gen_CustomerTable_All_Delete.empty:
                Gen_CustomerTable_All_Delete.dropna(how='all', inplace=True)
                Gen_CustomerTable_All_Delete.to_csv(
                    Outputdirectory + "Gen_CustomerTable_All_Delete_"+todaysDate+"_"+TokenDict["Gen_CustomerTable_Delete"]+".csv")
            if not Gen_ExportTerms_All_Delete.empty:
                Gen_ExportTerms_All_Delete.dropna(how='all', inplace=True)
                Gen_ExportTerms_All_Delete.to_csv(
                    Outputdirectory + "Gen_ExportTerms_All_Delete_"+todaysDate+"_"+TokenDict["Gen_ExportTerms_Delete"]+".csv")
            if not Gen_LocalSalesTerms_All_Delete.empty:
                Gen_LocalSalesTerms_All_Delete.dropna(how='all', inplace=True)
                Gen_LocalSalesTerms_All_Delete.to_csv(
                    Outputdirectory + "Gen_LocalSalesTerms_All_Delete_"+todaysDate+"_"+TokenDict["Gen_LocalSalesTerms_Delete"]+".csv")
            if not Gen_CountrywiseExport_All_Delete.empty:
                Gen_CountrywiseExport_All_Delete.dropna(
                    how='all', inplace=True)
                Gen_CountrywiseExport_All_Delete.to_csv(
                    Outputdirectory + "Gen_CountrywiseExport_All_Delete_"+todaysDate+"_"+TokenDict["Gen_CountrywiseExport_Delete"]+".csv")
            if not Gen_localPurchTerms_All_Delete.empty:
                Gen_localPurchTerms_All_Delete.dropna(how='all', inplace=True)
                Gen_localPurchTerms_All_Delete.to_csv(
                    Outputdirectory + "Gen_localPurchTerms_All_Delete_"+todaysDate+"_"+TokenDict["Gen_localPurchTerms_Delete"]+".csv")
            if not gen_ImportTerms_All_Delete.empty:
                gen_ImportTerms_All_Delete.dropna(how='all', inplace=True)
                gen_ImportTerms_All_Delete.to_csv(
                    Outputdirectory + "gen_ImportTerms_All_Delete_"+todaysDate+"_"+TokenDict["gen_ImportTerms_Delete"]+".csv")
            if not Gen_Countrywise_Purchase_All_Delete.empty:
                Gen_Countrywise_Purchase_All_Delete.dropna(
                    how='all', inplace=True)
                Gen_Countrywise_Purchase_All_Delete.to_csv(
                    Outputdirectory + "Gen_Countrywise_Purchase_All_Delete_"+todaysDate+"_"+TokenDict["Gen_Countrywise_Purchase_Delete"]+".csv")
            if not General_Telephone_All_Delete.empty:
                General_Telephone_All_Delete.dropna(how='all', inplace=True)
                General_Telephone_All_Delete.to_csv(
                    Outputdirectory + "General_Telephone_All_Delete_"+todaysDate+"_"+TokenDict["General_Telephone_Delete"]+".csv")
            if not Gen_SIC_All_Delete.empty:
                Gen_SIC_All_Delete.dropna(how='all', inplace=True)
                Gen_SIC_All_Delete.to_csv(
                    Outputdirectory + "Gen_SIC_All_Delete_"+todaysDate+"_"+TokenDict["Gen_SIC_Delete"]+".csv")
            if not History_Address_All_Delete.empty:
                History_Address_All_Delete.dropna(how='all', inplace=True)
                History_Address_All_Delete.to_csv(
                    Outputdirectory + "History_Address_All_Delete_"+todaysDate+"_"+TokenDict["History_Address_Delete"]+".csv")
            if not History_Background_All_Delete.empty:
                History_Background_All_Delete.dropna(how='all', inplace=True)
                History_Background_All_Delete.to_csv(
                    Outputdirectory + "History_Background_All_Delete_"+todaysDate+"_"+TokenDict["History_Background_Delete"]+".csv")
            if not History_auditor_All_Delete.empty:
                History_auditor_All_Delete.dropna(how='all', inplace=True)
                History_auditor_All_Delete.to_csv(
                    Outputdirectory + "History_auditor_All_Delete_"+todaysDate+"_"+TokenDict["History_auditor_Delete"]+".csv")
            if not History_StockExchange_All_Delete.empty:
                History_StockExchange_All_Delete.dropna(
                    how='all', inplace=True)
                History_StockExchange_All_Delete.to_csv(
                    Outputdirectory + "History_StockExchange_All_Delete_"+todaysDate+"_"+TokenDict["History_StockExchange_Delete"]+".csv")
            if not History_Authorize_All_Delete.empty:
                History_Authorize_All_Delete.dropna(how='all', inplace=True)
                History_Authorize_All_Delete.to_csv(
                    Outputdirectory + "History_Authorize_All_Delete_"+todaysDate+"_"+TokenDict["History_Authorize_Delete"]+".csv")
            if not History_Issued_All_Delete.empty:
                History_Issued_All_Delete.dropna(how='all', inplace=True)
                History_Issued_All_Delete.to_csv(
                    Outputdirectory + "History_Issued_All_Delete_"+todaysDate+"_"+TokenDict["History_Issued_Delete"]+".csv")
            if not History_Paidup_All_Delete.empty:
                History_Paidup_All_Delete.dropna(how='all', inplace=True)
                History_Paidup_All_Delete.to_csv(
                    Outputdirectory + "History_Paidup_All_Delete_"+todaysDate+"_"+TokenDict["History_Paidup_Delete"]+".csv")
            if not History_Sharehold_All_Delete.empty:
                History_Sharehold_All_Delete.dropna(how='all', inplace=True)
                History_Sharehold_All_Delete.to_csv(
                    Outputdirectory + "History_Sharehold_All_Delete_"+todaysDate+"_"+TokenDict["History_Sharehold_Delete"]+".csv")
            if not Mgmt_Loop_Non_Loop_All_Delete.empty:
                Mgmt_Loop_Non_Loop_All_Delete.dropna(how='all', inplace=True)
                Mgmt_Loop_Non_Loop_All_Delete.to_csv(
                    Outputdirectory + "Mgmt_Loop_Non_Loop_All_Delete_"+todaysDate+"_"+TokenDict["Mgmt_Loop_Non_Loop_Delete"]+".csv")
            if not Rel_Subsidiary_All_Delete.empty:
                Rel_Subsidiary_All_Delete.dropna(how='all', inplace=True)
                Rel_Subsidiary_All_Delete.to_csv(
                    Outputdirectory + "Rel_Subsidiary_All_Delete_"+todaysDate+"_"+TokenDict["Rel_Subsidiary_Delete"]+".csv")
            if not Rel_Affiliate_All_Delete.empty:
                Rel_Affiliate_All_Delete.dropna(how='all', inplace=True)
                Rel_Affiliate_All_Delete.to_csv(
                    Outputdirectory + "Rel_Affiliate_All_Delete_"+todaysDate+"_"+TokenDict["Rel_Affiliate_Delete"]+".csv")
            if not Rel_Branch_All_Delete.empty:
                Rel_Branch_All_Delete.dropna(how='all', inplace=True)
                Rel_Branch_All_Delete.to_csv(
                    Outputdirectory + "Rel_Branch_All_Delete_"+todaysDate+"_"+TokenDict["Rel_Branch_Delete"]+".csv")
            if not Bank_Loop_Non_Loop_All_Delete.empty:
                Bank_Loop_Non_Loop_All_Delete.dropna(how='all', inplace=True)
                Bank_Loop_Non_Loop_All_Delete.to_csv(
                    Outputdirectory + "Bank_Loop_Non_Loop_All_Delete_"+todaysDate+"_"+TokenDict["Bank_Loop_Non_Loop_Delete"]+".csv")
            if not Supplier_looping_All_Delete.empty:
                Supplier_looping_All_Delete.dropna(how='all', inplace=True)
                Supplier_looping_All_Delete.to_csv(
                    Outputdirectory + "Supplier_looping_All_Delete_"+todaysDate+"_"+TokenDict["Supplier_looping_Delete"]+".csv")
            if not Cinvest_Loop_All_Delete.empty:
                Cinvest_Loop_All_Delete.dropna(how='all', inplace=True)
                Cinvest_Loop_All_Delete.to_csv(
                    Outputdirectory + "Cinvest_Loop_All_Delete_"+todaysDate+"_"+TokenDict["Cinvest_Loop_Delete"]+".csv")
                                    ...
                                    try:
                                        ...
                                        if 'Delete' in file:
                                            sftp.put(os.path.join(
                                                Outputdirectory,  file),  confirm=False)
                                            # ,  confirm = False
                                            time.sleep(1)
                                            FTPStats.append(
            ...
            self.CreateStats(logger,  AutoFinancialObj,
                             StatsList, FTPStats, ValidationStatsList)
    ...
    def CreateStats(self, logger, AutoFinancialObj, StatsList, FTPList,  ValidationStatsList):
        ...
        try:
            book = load_workbook(
                Outputdirectory + 'AutoReportToDews_Stats.xlsx')
            ...
            book.save(Outputdirectory + 'AutoReportToDews_Stats_' +
                      str(today) + '.xlsx')
...
def main():
    ...
    LogStatsPath = settings_ReportToDews.get(
        'team_site_url') + '/Shared%20Documents/CBIG/Logs_Stats/'
    LogPath = settings_ReportToDews.get(
        'team_site_url') + '/Shared%20Documents/CBIG/Logs_Stats/'
    logger = loggerObj.setup_logger(
        'ReportToDewsLogs_',  LogFileReportToDews["LogFileReportToDews"] + logfilename)
    try:
        ...
        TotalElapsedTime = time.strftime(
            "%H:%M:%S %Z",  time.gmtime(ProcessEndTime - ProcessStartTime))
        ...
        if not Validation_Stats_Current.empty:
            ...
            output = build_table(df_stats,  'orange_light',  font_size='15px',
                                 text_align='center',  width='auto', font_family='Open Sans, sans-serif')
        ...
        logger.info(
            f"{TotalElapsedTime} seconds to process and upload all Report to Dews files")

        MessageUploadLog = Obj1.sharepointoperation("Upload File",
                                                    LogFileReportToDews["LogFileReportToDews"] +
                                                    logfilename,
                                                    logger,  "LOGS")
