...
def change_int_format(df, column_name):
    list_of_column = {'Length of relationship_General': 'Int64', 'Net days_General': 'Int64', 'To net days_General': 'Int64', 'Area code_General': 'Int64', 'Amount_History': 'Int64',
     'Post code_Relcon': 'Int64', 'Post code_Bank': 'Int64', 'Ord shares held(number)_History': 'Int64', 'Pref shares held (number)_History': 'Int64',
      'Affiliate owns shares in subject_Relcon': 'Int64', 'This affiliate is a joint venture_Recon': 'Int64', 'Affiliate DUNS_Relcon': 'Int64', 'DUNS_Relcon': 'Int64',
       'Ultimate parent DUNS_Relcon': 'Int64', 'Ordinary Shares (number)_History': 'Int64', 'Pref Shares (numbers)_History': 'Int64', 'Shares publicly listed_History': 'Int64',
        'Ordinary shares (Amount)_History': 'Int64', 'Par value_History': 'Int64', 'International_General': 'Int64', 'Total employees_General': 'Int64', 'Local_General': 'Int64',
         'Local purchase terms_General': 'Int64', 'Import terms_General': 'Int64', 'Local sales terms_General': 'Int64', 'Export sales terms_General': 'Int64',
...
def UpdateFieldIDDataframes(df, column_name):
    try:
        ...
        if "Mgmt" in  sheetname:
            ...
            #  Loops
            df['id'] =df.assign(temp=~df.duplicated(subset=['duns_no',column_name['Indian Bank Name_Bank']])).groupby('duns_no')[
                'temp'].cumsum()
            ...
            if 'SIC' in sheetname:
                df[column_name['SIC extension_General']] = df[column_name['SIC extension_General']].astype(str).apply(lambda x: x.zfill(4))
...
def validate_the_data(logger):
    ...
    if not os.path.exists (multicsv_delete) :
        os.mkdir (multicsv_delete)
    
    if not os.path.exists (multicsv_nondelete) :
        os.mkdir (multicsv_nondelete)
    ...
    for file in os.listdir(Inputdirectory):
        ...
        for record in records :
            ...
            if record[ 'type1' ] == 'single' :
                df.to_csv (OutPutFilesReportToDews['OutPutFilesReportToDews' ] + sheet_name + "_All_" + today + "_" + insert_token_id + ".csv" ,
                           index = False)
                if deletion == 'Y' :
                    df.dropna (how = 'all' , inplace = True)
                    df[ 'duns_no' ][ :1 ].to_csv (Outputdirectory + sheet_name + "_Delete_" + today + "_" + del_token_id + ".csv")
            elif record[ 'type1' ] == 'multi' :
                try:
                    
                    df.to_csv (multicsv_nondelete +'\\'+sheet_name+'_ALL_'+ "_"+"_" +insert_key + ".csv" ,index = False)
                except Exception as e:
                    print(e)
                if deletion == 'Y' :
                    df.dropna (how = 'all' , inplace = True)
                try:
                    df[ 'duns_no' ][ :1 ].to_csv (multicsv_delete +'\\'+sheet_name+'_All_'  + "_" +delete_key + ".csv")
                except :
                    pass
                ...
                status_list.append(["Failed", file_name, str(key), status,str(today) + "_" + str(log_time), "Worksheet is empty"])
